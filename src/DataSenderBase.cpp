...
DataSenderBase::DataSenderBase(const SocketAddress &sourceAddr, const SocketInfo &transportProtocolSocket,
    ...
    if (result > 0)
    {
        for (size_t i = 0; i < mReceivingSockets.size(); ++i)
        {
            ...
            else
            {
                ...
                {
                    throw std::runtime_error("Exception occurred while polling: got revents: " +
                                             std::to_string(revents));
                }
            }
        }
    }
...
SocketInfo createIcmpSocket(const SocketAddress &addressToBind)
{
    ...
    {
        throw std::runtime_error("Could not bind address: " + addressToBind.toString() +
